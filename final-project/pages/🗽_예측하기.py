...
def load_model():
    ...
    state_dict = torch.utils.model_zoo.load_url(
        'https://drive.google.com/uc?export=download&id=1-3SvCFcqdaecIZzziq6PnfFSKFj7n3Os&confirm=t', map_location=torch.device('cpu'))
    model.load_state_dict(state_dict['net'])
    return model


albumentations_test = A.Compose(
    [A.Resize(128, 128, always_apply=True), A.Normalize(), ToTensorV2()])
...
def main():
    ...
    with empty1:
        empty()  # 여백부분1
        st.sidebar.markdown("이미지를 넣어서 예측해보세요!")

    with con1:
        st.markdown("### 테마 분류 해보기")
    ...
    with con3:
        if image_file is not None:
            ...
            vali_dataset = CustomDataset(
                I, [0], train_mode=True, transforms=albumentations_test)
            vali_loader = DataLoader(
                vali_dataset, batch_size=16, shuffle=False, num_workers=0)
